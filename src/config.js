...
function ensureEnvFileWithMessaging() {
  ...
  if (!fs.existsSync(envPath)) {
    ...
    // .env exists — ensure MAP_KEY is present
    const content = fs.readFileSync(envPath, "utf-8");
    if (!content.includes("MAP_KEY=")) {
      const key = generateMapKey();
      fs.appendFileSync(envPath, `\nMAP_KEY=${key}\n`);
      addedKey = true;
      console.log(`✔ Added MAP_KEY to existing .env: ${envPath}`);
    } else {
      console.log(`ℹ Using existing MAP_KEY from: ${envPath}`);
    }
  }
  ...
  console.log("• Keep your MAP_KEY safe — it decrypts your source code.");
  console.log("• NEVER commit .env to Git (add it to .gitignore).");
  console.log("• If you lose MAP_KEY, you cannot unpack encrypted files.\n");
}
...
function ensurePackageScripts() {
  ...
  if (changed) {
    ...
    console.log("✔ Added zozzona scripts to package.json");
  }
}
